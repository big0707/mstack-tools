...
$markerPath = Join-Path $venvPath ".gtm-mcp-ready"
...
function Write-Diagnostic([string]$Message) {
    [Console]::Error.WriteLine("[gtm-mcp] $Message")
}
...
if ($needsInstall) {
    Write-Diagnostic "Installing GTM MCP dependencies. This only happens on first launch or after a dependency update."
...
& $venvPython -m gtm_agent.mcp_server
exit $LASTEXITCODE
